...
export default function Destinations() {
  ...
  const { data, isLoading, isFetching, isError } = useQuery({
    ...
    placeholderData: keepPreviousData, // keeps current results visible while the next page loads — no flash of empty/loading state
  });
    ...
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-full border border-gray-300 py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <select
          ...
          className="rounded-full border border-gray-300 px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      {!isLoading && data?.data?.length > 0 && (
        <div className={`grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 transition-opacity ${isFetching ? "opacity-60" : ""}`}>
          {data.data.map((dest) => <DestinationCard key={dest._id} destination={dest} />)}
        </div>
      )}
